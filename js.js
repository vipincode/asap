...
jQuery(window).scroll(function () {
    ...
    jQuery(document).on('click', '.sidebar-btns', function () {
      ...
      jQuery.ajax({
        ...
        success: function (t) {
          ...
          s.forEach((t) => {
            if (t.name !== 'Refresh' && t.name !== 'Invisible fresh') {
            ...
            jQuery.each(a, function (t, e) {
              ...
              jQuery.each(e.meta_data, function (t, e) {
                if ('product_key' === e.key) {
                  a[e.key] = e.value;
                  // Convert product key to lowercase for comparison
                  const productKeyLowerCase = e.value.toLowerCase();
                  if (productKeyLowerCase === 'refresh' || productKeyLowerCase === 'invisible fresh') {
                    isValidProduct = false;
                  }
                } else if ('product_cat' === e.key) {
                  // Check product category
                  // Assuming e.value is an array of category IDs
                  if (e.value.includes('CATEGORY_ID_TO_EXCLUDE')) {
                    isValidProduct = false;
                  }
                } else if ('quantity_&_price' === e.key) {
                  a.product_quantity = e.value;
                }
              });
              // Check product categories
              if (e.categories && e.categories.length > 0) {
                e.categories.forEach(function (category) {
                  // Assuming category.slug is the category slug
                  if (category.slug === 'refresh') {
                    isValidProduct = false;
                  }
                  if (category.slug === 'invisible-fresh') {
                    isValidProduct = false;
                  }
                });
              }
  ...
  jQuery(document).ready(function () {
    ...
    // 	$(".sidebar-btns:first-child").addClass('sidebar-btns-active');
    // 	$(".product-main-content").on('click', '.sidebar-btns', function() {
    // 		$(".sidebar-btns:first-child").removeClass('sidebar-btns-active');
    // 		$(this).addClass('sidebar-btns-active');
    // 	})
  }),
